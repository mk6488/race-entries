...
function verifyPin(pin, secret) {
  const { saltHex, iterations, keylen, digest, hashHex } = secret;
  const derived = crypto.pbkdf2Sync(String(pin), Buffer.from(saltHex, "hex"), iterations, keylen, digest);
...
function requireNonEmpty(value, fieldName) {
  ...
  if (!v) {
    throw new functions.https.HttpsError("invalid-argument", `Missing or empty field: ${fieldName}`);
  }
  return v;
}
 ...
 */
exports.createCoachProfile = functions
  .region(REGION)
 ...
 */
exports.linkDeviceToCoach = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    ...
    if (qs.size > 1) {
      throw new functions.https.HttpsError("failed-precondition", "Multiple coach profiles match that name.", {
    ...
    if (!secretSnap.exists) {
      throw new functions.https.HttpsError("failed-precondition", "Coach PIN is not set up. Contact an admin.");
    }
